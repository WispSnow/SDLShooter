**简体中文** | [English](./README.md)

# SDLShooter

**SDLSHOOTER** 是一个使用 C++ 和 SDL2 (SDL3) 开发的跨平台太空射击游戏。

> 本项目是一个教学演示项目，是系列教程 “[C++ 游戏开发之旅](https://cppgamedev.top/)” 的第一章。

## 操作说明
```
W,A,S,D - 移动
J - 攻击
F4 - 切换全屏
ESC - 返回标题
```

## 网页试玩
[SDLShooter](https://wispsnow.github.io/SDLShooter/)

## 截图

<div style="display: flex; gap: 10px;">
  <img src="https://theorhythm.top/gamedev/screen_shooter_1.png" width="300" />
  <img src="https://theorhythm.top/gamedev/screen_shooter_2.png" width="300" />
</div>
<div style="display: flex; gap: 10px;">
  <img src="https://theorhythm.top/gamedev/screen_shooter_3.png" width="300" />
  <img src="https://theorhythm.top/gamedev/screen_shooter_4.png" width="300" />
</div>


## 第三方库
* [SDL2](https://github.com/libsdl-org/SDL)
* [SDL2_image](https://github.com/libsdl-org/SDL_image)
* [SDL2_mixer](https://github.com/libsdl-org/SDL_mixer)
* [SDL2_ttf](https://github.com/libsdl-org/SDL_ttf)

查看 [wiki](../../wiki) 获取详细构建指南（中文）。

## 鸣谢 / 资源来源
- 图片:
  - https://anim86.itch.io/space-shoter-starter-pack
  - https://doshrosity.itch.io/32x32-health-ui
- 音效:
  - https://enprimer.itch.io/spaceship-sound-effects-pack
- 音乐:
  - https://leohpaz.itch.io/space-music-pack
- 字体:
  - https://timothyqiu.itch.io/vonwaon-bitmap
- 特效:
  - https://ansimuz.itch.io/explosion-animations-pack

- 赞助者: `sino`, `李同学` 

## 请我喝咖啡
<a href="https://ko-fi.com/ziyugamedev"><img src="https://storage.ko-fi.com/cdn/kofi2.png?v=3" alt="Buy Me A Coffee" width="200" /></a>
<a href="https://afdian.com/a/ziyugamedev"><img src="https://pic1.afdiancdn.com/static/img/welcome/button-sponsorme.png" alt="Support me on Afdian" width="200" /></a>

## QQ 交流群与微信二维码

<div style="display: flex; gap: 10px;">
  <img src="https://theorhythm.top/personal/qq_group.webp" width="200" />
  <img src="https://theorhythm.top/personal/wechat_qr.webp" width="200" />
</div>
